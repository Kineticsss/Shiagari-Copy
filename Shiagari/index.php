...
if (isset($_SESSION['uid']) && $_SESSION['uid'] !== '') {
    // Prevent redirect loop - only redirect if not already attempting to
...
<div id="loginModal" class="modal">
    ...
    <div class="modal-content">
        ...
        <div class="modal-switch">
            <a href="auth/forgot-password.php" style="font-size: 12px;">Forgot Password?</a>
        </div>
        <div class="modal-switch">
...
<script>
    ...
    async function submitAuthForm(endpoint, payload, button) {
        ...
        try {
            ...
            try {
            // Note: api/user-profile.php expects GET + optional ?uid/token; server auth middleware
            // will validate from session. Use absolute relative URL from root.
            const profileResponse = await fetch('api/user-profile.php', {
                ...
                if (profileResponse.ok) {
                    ...
                    if (profileData.success && profileData.profile) {
                        // Store verified profile data (cache only; UI should fetch from server)
                        localStorage.setItem('shiagari_user_uid', profileData.profile.uid || '');
                        localStorage.setItem('shiagari_user_email', profileData.profile.email || '');
                        localStorage.setItem('shiagari_user_full_name', profileData.profile.fullName || payload.full_name || '');
                        localStorage.setItem('shiagari_user_username', profileData.profile.username || payload.username || '');
                        localStorage.setItem('shiagari_user_created_at', profileData.profile.createdAt || new Date().toISOString());
                        localStorage.setItem('shiagari_user_verified', 'true');
                    }
                }
            } catch (profileError) {
                console.warn('Could not fetch user profile:', profileError);
                // Still proceed with login, profile will be fetched on next page load
            }
            ...
            // Only redirect to valid PHP routes.
            const target = (typeof result.redirect === 'string' && result.redirect.trim() !== '')
                ? result.redirect
                : 'landing/landing.php';

            if (target.includes('landing/landing.html')) {
