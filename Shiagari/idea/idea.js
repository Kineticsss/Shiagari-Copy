...
function getStorageKey(projectId) {
  return `shiagari_ideas_${projectId}`;
}

function saveToStorage(projectId) {
  localStorage.setItem(getStorageKey(projectId), JSON.stringify(ideas));
...
function render() {
  ...
  document.querySelectorAll('.like-idea').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (idea) { idea.likes++; saveToStorage(getSelectedProjectId()); render(); showToast(`❤️ Liked "${idea.title}"`); }
    });
  });
  ...
  document.querySelectorAll('.delete-idea').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (idea && confirm(`Delete "${idea.title}"?`)) {
        ...
        saveToStorage(getSelectedProjectId());
        render();
...
function init() {
  loadFromStorage(getSelectedProjectId());
  render();
  initProjectSelector();
