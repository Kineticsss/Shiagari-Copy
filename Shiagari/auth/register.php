...
function registerUser(string $email, string $password, string $fullName = '', string $username = ''): array {
    ...
    if (isset($result['idToken'])) {
        ...
        if ($profileResult['success']) {
            ...
            // Profile save failed, but user is authenticated
            error_log('Failed to save user profile: ' . json_encode($profileResult));
            return ['success' => true, 'uid' => $uid, 'warning' => 'Profile not fully saved'];
        }
    }
