...
function registerUser(string $email, string $password, string $fullName = '', string $username = ''): array {
    ...
    if (isset($result['idToken'])) {
        ...
        $profileResult = save_user_profile($uid, $result['email'], $fullName, $username, $token);
        
        if ($profileResult['success']) {
            return ['success' => true, 'uid' => $uid];
        } else {
            // Profile save failed - log the error
            error_log('Failed to save user profile for UID: ' . $uid . ' - Error: ' . json_encode($profileResult));
            // Return success anyway since Firebase account was created
